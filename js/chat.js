document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderChatList(chats) {
    ...
    chatList.innerHTML = chats.map(chat => {
      const avatar  = chat.friend_avatar ? `https://mundialfan-api.railway.internal/uploads/${chat.friend_avatar}` : '../images/default-profile.jpg';
  ...
  async function openChat(friendId) {
    ...
    const avatar = currentChatUser.friend_avatar
      ? `https://mundialfan-api.railway.internal/uploads/${currentChatUser.friend_avatar}`
  ...
  function renderMessage(msg) {
    ...
    if (msg.media_url) {
      ...
      if (msg.media_type === 'image') {
        mediaHtml = `<div class="msg-media mb-1" style="cursor:pointer;" onclick="openMediaViewer('msg_${msg.id}')"><img src="https://mundialfan-api.railway.internal/uploads/${msg.media_url}" style="max-width:100%; border-radius:8px;"></div>`;
      } else if (msg.media_type === 'video') {
        mediaHtml = `<div class="msg-media mb-1" style="cursor:pointer;" onclick="openMediaViewer('msg_${msg.id}')"><video controls src="https://mundialfan-api.railway.internal/uploads/${msg.media_url}" style="max-width:100%; border-radius:8px;"></video></div>`;
      }
    }
  ...
  function renderMediaGroup(group) {
    ...
    for (let i = 0; i < displayCount; i++) {
      ...
      if (item.media_type === 'image') {
        mediaTag = `<img src="https://mundialfan-api.railway.internal/uploads/${item.media_url}">`;
      } else if (item.media_type === 'video') {
        mediaTag = `<video src="https://mundialfan-api.railway.internal/uploads/${item.media_url}"></video><div class="msg-mosaic__video-icon"><i class="fas fa-play"></i></div>`;
      }
  ...
  window.openMediaViewer = function (mediaId) {
    ...
    content.innerHTML = items.map(item => {
      if (item.media_type === 'image') return `<img src="https://mundialfan-api.railway.internal/uploads/${item.media_url}" class="media-viewer__item">`;
      if (item.media_type === 'video') return `<video controls src="https://mundialfan-api.railway.internal/uploads/${item.media_url}" class="media-viewer__item"></video>`;
      return '';
    }).join('');
